;(function() {
  ...
  Game.prototype.makeVulnerable = function (ship) {
    setInterval(function(){ship.isVulnerable = true;},SpaceInvaders.Game.RESPAWN_TIME);
  };
  ...
  Game.prototype.addObject = function (object) {
    ...
    }else if (object instanceof SpaceInvaders.Explosion){
  ...
  Game.prototype.fireBossBullets = function () {
    ...
    if (boss.health <= this.wave.health){//If enemies health is less than 4, fire seven shots
      if (currentFrame > (SpaceInvaders.GameView.FPS/this.wave.fireFreq)/1.5){
        this.enemyShips.forEach(function(ship, idx){
      ...
      var burstPeriod = SpaceInvaders.GameView.FPS/this.wave.fireFreq;
      var firePeriod = burstPeriod+(SpaceInvaders.GameView.FPS/this.wave.fireFreq*3);
      var pausePeriod = SpaceInvaders.GameView.FPS/this.wave.fireFreq*3;
      var totalPeriod = firePeriod + pausePeriod;

      var burstFreq = parseInt(burstPeriod/5);
      var fireFreq = parseInt((SpaceInvaders.GameView.FPS/this.wave.fireFreq)/1.5);
  ...
  Game.prototype.drawHeader = function () {
    ...
    ctx.textAlign = 'left';
    ctx.fillText("Score: ", SpaceInvaders.Game.DIM_X - 150, 20);
    ctx.fillText(this.displayScore(), SpaceInvaders.Game.DIM_X - 150 + 55, 20);
    ctx.fillText("Lives: ", 50, 20);
    ctx.fillText(this.lives, 50 + 55, 20);

    ctx.fillStyle = '#000000';
  };

  Game.prototype.drawSmallExplosion = function (pos) {
    this.addObject(new SpaceInvaders.Explosion({pos: pos, width: 12, height: 12, frames: 7, radius: 10, ticksPerFrame: 2, src: './assets/smallExplosion.png'}));
  };

  Game.prototype.drawLargeExplosion = function (pos, rad) {
    this.addObject(new SpaceInvaders.Explosion({pos: pos, width: 66, height: 66, frames: 10, radius: rad, ticksPerFrame: 2, src: './assets/largeExplosion.png'}));
  };
